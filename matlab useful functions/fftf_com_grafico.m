...
switch unit
    case 'acel'
        ...
        ylabel('Aceleração (mag)');
        title({'FFT (Espectro de frequência)',strcat('Fs = ',num2str(Fs),' (Hz)')});
        grid on;
        title('Espectro aceleração');
        hold on;

    case 'vel'
        figure
        loglog(freq_vector,velocidade,'linewidth',1.5);
        xlabel('Frequencia (Hz)');
        ylabel('Velocidade (mag)');
        title({'FFT (Espectro de frequência)',strcat('Fs = ',num2str(Fs),' (Hz)')});
        grid on;
        title('Espectro Velocidade');
        hold on;
        magnitude_output = velocidade;
    otherwise
        figure
        loglog(freq_vector,deslocamento,'linewidth',1.5);
        xlabel('Frequencia (Hz)');
        ylabel('Deslocamento (mag)');
        title({'FFT (Espectro de frequência)',strcat('Fs = ',num2str(Fs),' (Hz)')});
        grid on;
        title('Espectro deslocamento');
        hold on;
